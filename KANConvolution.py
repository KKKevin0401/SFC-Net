...
def calc_out_dims(matrix, kernel_side, stride, dilation, padding):
    ...
    h_out = np.floor((n + 2 * padding[0] - kernel_side - (kernel_side - 1) * (dilation[0] - 1)) / stride[0]).astype(
        int) + 1
    w_out = np.floor((m + 2 * padding[1] - kernel_side - (kernel_side - 1) * (dilation[1] - 1)) / stride[1]).astype(
        int) + 1
    b = [kernel_side // 2, kernel_side // 2]
    return h_out, w_out, batch_size, n_channels


def kan_conv2d(matrix, kernel, kernel_side, stride, dilation, padding, device):
...
def multiple_convs_kan_conv2d(matrix, kernels, kernel_side, stride, dilation, padding, device):
    h_out, w_out, batch_size, n_channels = calc_out_dims(matrix, kernel_side, stride, dilation, padding)
    n_convs = len(kernels)
