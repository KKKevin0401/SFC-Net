...
from KANConvolution import KAN_Convolutional_Layer
from torch.utils.checkpoint import checkpoint

from models.archs.attention_modules import OGWA
class HighFrequencyEnhancer(nn.Module):
    def __init__(self, channels=3):
        ...
        self.kan_layer1 = create_kan_layer(3, [-0.5, 0.5],0.5)
    ...
    def forward(self, x):
        ...
        if self.training and hf.requires_grad:
            out = checkpoint(self.kan_layer1, hf, use_reentrant=False)
        else:
...
class ProcessBlock(nn.Module):
    def __init__(self, dims, d_state=16, n_l_block=1, n_h_block=1, LayerNorm_type='WithBias'):
        ...
        self.enhance_hl = OGWA(self.dim, mode='direction', direction='h')

        self.enhance_lh = OGWA(self.dim, mode='direction', direction='v')
        

        self.enhance_hh = OGWA(self.dim, mode='sparse')
        ...
        self.post_iwt_enhancer = HighFrequencyEnhancer(channels=self.dim)
        # ==============================================================
    ...
    def forward(self, x):
        ...
        hh_cat = torch.cat((hl, lh, hh), dim=0) 
        hh_conv = self.hhenhance(hh_cat)

        hl_conv, lh_conv, hh_conv = hh_conv.chunk(3, dim=0) 

        e_hl = self.enhance_hl(hl_conv)
        

        e_lh = self.enhance_lh(lh_conv)

        e_hh = self.enhance_hh(hh_conv)
        ...
        x_out = self.post_iwt_enhancer(x_out)
        # ============================================================
